...
class PodpisApp:
    ...
    def _create_info_tab(self):
        ...
        # dół prawa strona
        self.RightFrame = customtkinter.CTkFrame(self.InfoFrameDOWN, fg_color="transparent", border_width=1)
        ...
        self.LeftFrame = customtkinter.CTkFrame(self.InfoFrameDOWN, fg_color="transparent", border_width=1, height=80, width=220)
    ...
    def generate_sign(self):
        ...
        c = rcanvas.Canvas("sig.pdf", pagesize=(page_width, page_height))
        ...
        # zapisanie pliku z podpisem - sig.pdf
        c.save()

        # Scalanie PDF orig i podpisanego sig.pdf
        pdf_path = Path(self.EntryORIG_PDF.get())

        with pikepdf.open(pdf_path) as original, pikepdf.open("sig.pdf") as sig:
            page = original.pages[self.page_index]
            # Dodajemy podpis jako obrazek na stronie
            page.add_overlay(sig.pages[0])
            original.save("signed_output.pdf")

        # sprzątanie
        sig_file = Path("sig.pdf")
        if sig_file.exists():
            sig_file.unlink()
    ...
    def electronic_sign(self):
        ...
        if Path("signed_output.pdf").exists():
            SIG_PDF = "signed_output.pdf"
        else:
            SIG_PDF = ORIG_PDF
        ...
        if sig_file.exists():
            sig_file.unlink()
        sig_file = Path("signed_output.pdf")
        if sig_file.exists():
            sig_file.unlink()
        # Zapisanie ustawień
